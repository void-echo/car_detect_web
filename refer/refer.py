...
class HumanMachineSafety:
    ...
    # calc_spatials方法根据深度图和边界框（ROI）计算空间坐标。它首先根据边界框的大小调整ROI的大小，
    # 然后计算ROI中的深度平均值。最后，根据计算得到的深度和角度，计算出空间坐标（X、Y、Z）。
    ...
    def draw_palm_detection(self, palm_coords, depth):
        ...
        for bbox in palm_coords:
            ...
            if spatialCoords is None or any(math.isnan(coord) for coord in spatialCoords):  # Check for NaN values
                continue
    ...
    # 它解析手掌检测、目标检测和深度图输出，并调用其他方法进行绘制和计算。它首先通过手掌检测模型获取手掌的边界框，并计算手掌的空间坐标。
    # 然后，根据目标检测结果，计算目标物体与手掌之间的距离，并在图像上显示相关信息。
